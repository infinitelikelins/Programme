...
public class StationActionFragment extends BaseFragment implements View.OnClickListener, View.OnLongClickListener {
    ...
    private TextView[] secondView = new TextView[3];

    private TextView currentEditSecondView;

    private static Map<Integer, Integer> ACTION_IMAGE_MAP = new ArrayMap<>();
    ...
    public static StationActionFragment newInstance() {
        return new StationActionFragment();
    }
    ...
    protected void loadLastStationConfig() {
        ...
        if (station != null && station.actions != null && station.actions.size() > 0) {
            ...
            if (actionMap.indexOfKey(1) >= 0 && actionMap.get(1) != null) {
                setActionToView(actionMap.get(1), box1, secondView[0]);
            }
            if (actionMap.indexOfKey(2) >= 0 && actionMap.get(2) != null) {
                setActionToView(actionMap.get(2), box2, secondView[1]);
            }
            if (actionMap.indexOfKey(3) >= 0 && actionMap.get(3) != null) {
                setActionToView(actionMap.get(3), box3, secondView[2]);
            }
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1009 && resultCode == Activity.RESULT_OK) {
            ...
            if (content != null && TextUtils.isDigitsOnly(content)) {
                try {
