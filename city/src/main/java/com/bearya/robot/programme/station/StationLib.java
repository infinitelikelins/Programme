...
public class StationLib {
    ...
    public static StationLib getLibsFromAssets(Context context) {
        try {
            String json = BaseApplication.isEnglish ? "station/en/station_libs.json" : "station/zh/station_libs.json";
            JSONObject root = new JSONObject(FileUtil.stringFromAssetsFile(context, json));
    ...
    private static Lib parseSoundLib(JSONObject json) throws JSONException {
        ...
        for (int i = 0; i < itemArr.length(); i++) {
            ...
            if (imageType.equals("res")) {
                ...
                soundItem.image = "" + ResourceUtil.getMipmapId(imageName + itemJson.optString("index"));
            }
